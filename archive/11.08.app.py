...
def serve_upload(filename):
    # optional: leite auf /static/uploads/... um
...
def extract_text_from_file(path: str) -> str:
    ...
    if ext in [".jpg", ".jpeg", ".png", ".webp", ".bmp", ".tif", ".tiff"]:
        ...
        texts = []
        for p in pages[:3]:
            texts.append(pytesseract.image_to_string(p, lang="deu+eng"))
        return "\n".join(texts)
    else:
...
def extract_multiple_events(text: str):
    ...
    location = ""
    if re.search(r"Theater\s*Brand", text, re.I):
        location = "Theater Brand, Aachen"
    ...
    for d in sorted(set(dates)):
        dt = f"{d} {time_str}" if time_str else d
        events.append({
            "title": title,
            "date": dt,
...
def is_admin():
    try:
        return bool(current_user.is_admin)  # oder deine Admin-Logik
    except:
        return False
...
def admin_diag():
    ...
    def cmd_version(bin):
        ...
        try:
            out = subprocess.check_output([p, "--version"], stderr=subprocess.STDOUT, text=True)[:200]
    ...
    diag = {
        ...
        "db_url": str(engine.url).split("@")[0] + "@…",  # keine Secrets leaken
        "ocr_langs_expected": "deu+eng"
    }
    return diag
